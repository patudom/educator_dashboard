...
class Roster():
    ...
    def progress_summary(self):
        ...
        return {
            'progress': completion_string,
            'percent_story_complete': completion_percent
        }
    ...
    def hubble_data(self):
        ...
        return {
            'hubble_constant': summary['H0'].apply(lambda x: round(x,2)) if 'H0' in summary else None,
            'age': summary['age'].apply(lambda x: round(x,2)) if 'age' in summary else None
        }
    ...
    def last_modified_formatted(self):
        ...
        if not hasattr(self, 'last_modified') or not self.last_modified:
            return None
        return pd.to_datetime(self.last_modified['last_modified']).tz_convert('US/Eastern').strftime("%Y-%m-%d %H:%M:%S (Eastern)")
    ...
    def report(self, refresh = False, for_teacher = True):
        ...
        if hasattr(self,'stages') and (not for_teacher):
            data = [[get_or_none(s, 'marker', None) for s in stage] for stage in self.stages]
        ...
        response = flatten(pd.DataFrame(self.story_state['responses']))
        response['student_id'] = self.student_id['student_id']
        df = df.merge(response, on='student_id', how='left')
        last_modified = pd.to_datetime(self.last_modified['last_modified']).tz_convert('US/Eastern').strftime("%Y-%m-%d %H:%M:%S (Eastern)") # in Easterm time
