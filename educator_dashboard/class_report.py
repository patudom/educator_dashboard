...
class Roster():
    ...
    def fix_new_story_state(self) -> None:
        """
        ...
        for i, student in enumerate(self.roster):
            ...
            self.roster[i]['app_state'] = cast(Dict, story_state['app'])
            
            # Replace story_state with just the story part
            self.roster[i]['story_state'] = story_state['story'] # type: ignore -- we don't use the mis-matched parameters
            ...
            if 'responses' in free_responses_dict:
                ...
                cast(OldStudentStoryState, self.roster[i]['story_state'])['responses'] = responses
            
            # Remove free_responses after extraction
            if 'free_responses' in self.roster[i]['story_state']:
    ...
    def grab_data(self) -> None:
        ...
        if self.new_db:
            self.roster = cast(List[NewRosterEntry], api_roster)
        else:
            self.roster = cast(List[OldRosterEntry], api_roster)
        
        # Process data for new database format
        ...
        new_out = l2d(self.roster) # type: ignore
        keys = new_out.keys()
        for key in keys:
